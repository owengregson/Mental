...
| `signature` | **Mental's own**, derived from `velt`: the same residual wipe, pinned 0.36 vertical and full 0.5 sprint horizontal, but `air.horizontal 0.92` — airborne combo hits (the second and later) are trimmed 8% so the victim holds the reach pocket instead of drifting out. The grounded opener is byte-identical to velt. | Original Mental tuning (a velt derivative) |
