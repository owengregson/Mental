...
public final class BootSuite {
    ...
    private static final List<String> EXPECTED_MODULES = List.of(
            "anticheat-compat", "hit-registration", "knockback", "latency-compensation",
    ...
    public static @NotNull List<TestCase> tests(@NotNull MentalPlugin mental) {
        return List.of(
                new TestCase("boot: every module registered and active", context -> {
                    for (String id : EXPECTED_MODULES) {
                        var module = mental.modules().byId(id).orElse(null);
                        context.expect(module != null, "module '" + id + "' is not registered");
                        context.expect(module.active(), "module '" + id + "' is not active");
                    }
                }),
                new TestCase("boot: capability report matches the booted version", context -> {
                    var capabilities = mental.services().capabilities();
                    var environment = mental.services().environment();
                    ...
                    if (capabilities.folia()) {
                        ...
                        context.expect("folia".equals(mental.services().scheduling().describe()),
                                "folia must select the folia scheduling backend");
                    } else {
                        context.expect("bukkit".equals(mental.services().scheduling().describe()),
