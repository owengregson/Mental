 ...
 * </ul>
 */
public final class FeedbackPlan {
    ...
    public static List<Step> plan(
        ...
        if (!includeVelocity) {
            return List.of(Step.HURT);
        }
        if (bundleWanted && bundleCapable) {
            return List.of(Step.BUNDLE_OPEN, Step.VELOCITY, Step.HURT, Step.BUNDLE_CLOSE);
        }
        return List.of(Step.VELOCITY, Step.HURT);
    }
}
