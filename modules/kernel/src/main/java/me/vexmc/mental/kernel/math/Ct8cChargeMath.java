...
public final class Ct8cChargeMath {
    ...
    /** The default lenient miss-recovery partial-recharge gate: {@code startValue − ticker > 4.0} (spec §2.1). */
    public static final double MISS_RECOVERY_TICKS = 4.0;

    private Ct8cChargeMath() {}

    /**
     * Whether an attack may land against a <em>configured</em> miss-recovery gate
     * — fully charged ({@code scale ≥ 1}), OR the miss-recovery lane is open (an
     * air swing set {@code missRecovery} and more than {@code missRecoveryTicks}
     * ticks have elapsed). {@link #attackAllowed(double, boolean, double)} is the
     * {@link #MISS_RECOVERY_TICKS default-gate} shorthand.
     */
    public static boolean attackAllowed(
            double scale, boolean missRecovery, double ticksSinceReset, double missRecoveryTicks) {
        return scale >= 1.0 || (missRecovery && ticksSinceReset > missRecoveryTicks);
    }
    ...
    /**
     * Whether an attack may land at the default four-tick miss-recovery gate: fully
     * charged ({@code scale ≥ 1}), OR the miss-recovery lane is open (an air swing
     * set {@code missRecovery} and more than four ticks have elapsed) — spec §2.1.
     */
    public static boolean attackAllowed(double scale, boolean missRecovery, double ticksSinceReset) {
        return attackAllowed(scale, missRecovery, ticksSinceReset, MISS_RECOVERY_TICKS);
    }
