...
- **Shields** — instant, a 148° protection cone, cap 5 damage per melee hit
  with the excess passing through, crouching blocks (damage-math) with an
  offhand shield on the ground, 50% knockback resistance while blocking, axes
  always disable for 1.6s (+0.5s per Cleaving level) and cost 1 durability.
- **Food** — 1.8-style regen: +1 HP every 2s above 3 shanks, a 50% chance to
  ...
  by a player or mob interrupts eating.
...
- **Projectiles** — snowballs and eggs knock players (0 damage, full 0.4
  ...
  drawn (shakes off critical arrows), thrown-projectile momentum only in the
  aim direction, Loyalty tridents return from the void.
...
- The 80–100% charge click buffer (`retainAttack`).
...
- Potion stacking, 20-tick drinks, snowball stacking, eat-interrupt: 1.20.5+.
