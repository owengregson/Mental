...
/**
 * A fluent, cross-version menu-icon builder.
 *
 * <p>Everything here is binary-safe on the 1.17.1 floor (verified against the
 * floor API): the Adventure {@code displayName(Component)} / {@code
 * lore(List&lt;Component&gt;)} meta methods, {@link ItemFlag#HIDE_ENCHANTS} and
 * {@link ItemFlag#HIDE_ATTRIBUTES}. Materials are resolved through {@link
 * Materials} so a constant renamed on a future year-scheme server degrades to
 * a safe staple instead of failing to link.</p>
 *
